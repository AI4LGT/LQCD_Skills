...
HARD REQUIREMENT (Dirac loading before inversion): before every propagator
solve call (`core.invert`, `core.invertPropagator`, `core.invertSequential`),
you must first load the matching Dirac gauge context via
`with dirac.useGauge(gauge_for_this_dirac):`.
Never call inversion routines outside this context. If you temporarily switch
to another gauge context (for example `pure_gauge.covDev` for Wilson-line
shift), re-enter the correct `with dirac.useGauge(...)` block before the next
inversion.
...
Memory rule (GPU, HARD REQUIREMENT): do not pre-build and keep all propagators
for all `t_src` outside the loop. Construct the propagator inside the `t_src`
loop, immediately perform the needed contraction/output, then release temporary
objects before moving to the next source time. This keeps peak GPU memory low
and avoids OOM on large lattices.

The same rule applies to wall-source momentum runs (HARD REQUIREMENT): do not
pre-build propagators for all momenta outside the loop. For each `t_src` and
each momentum, build only the current momentum propagator, contract/save, then
release temporary objects before the next momentum. Avoid holding a full
`{momentum: propagator}` map in GPU memory.

Hard fail pattern (forbidden in generated code): building momentum propagator
lists/maps such as `forward_props`, `backward_props`, or `{mom: propagator}`
first, and contracting in a separate later loop.
...
- `gamma_src` and `gamma_snk` should store only the physical bilinear operators
  inserted at source and sink
...
- those hermiticity factors appear only in the contraction formula, typically as
  `gamma_5 @ gamma_snk` on the sink side and `gamma_src.conj().T @ gamma_5` on
  the source side

For example, if the physical channel is `gtg5`, then define
`gamma_src = gamma_snk = gamma_t @ gamma_5` as the operator itself. Do not fold
the additional hermiticity `gamma_5` factors into these definitions.
