...
**Initialization order is mandatory (hard rule)**: In generated scripts, always call `core.init(...)` before any gauge I/O call such as `io.readChromaQIOGauge`, `io.readMILCGauge`, or other `io.read*Gauge`. Never read gauge files before MPI/grid initialization. The required top-level order is: parameters -> `core.init` + `LatticeInfo` -> gauge read -> Dirac setup -> inversion/contraction. Violating this order can raise `RuntimeError: Grid is not initialized` from `pyquda_comm`.
...
This step is a strict prerequisite for gauge reading. Do not call any `io.read*Gauge(...)` before `core.init(...)`.
...
```
Here we call the `invert` function to solve for the propagator using different source types. For a point source, we specify the position `[x0, y0, z0, t0]`. For a wall source, we specify the time slice `t0`. For a volume source, no additional parameters are needed. Here we also apply a momentum phase to the source, which is necessary for computing momentum wall source propagators. If no `phase` is given, the values for all three types will default to 1. For momentum-projected wall-source 2pt, follow the two-propagator rule in the next subsection.

#### Wall source momentum 2pt: two propagators + three-phase bookkeeping

For **momentum-projected 2pt with wall source**, solve **two wall propagators** and contract in the form `S2^dagger ... S1`.

This is a hard rule, not an optimization choice:

- Do **not** replace this with a single wall propagator plus only sink Fourier phase.
- For each extracted momentum `P`, build a dedicated pair `(p1, p2)` with `P = p1 - p2`, then invert both lines.
- If multiple extracted momenta are requested (for example `Pz=0` and `Pz=4`), compute one two-line channel per requested momentum.
- Even for `P=0`, keeping the same two-line structure (`p1=p2=0`) is recommended for consistent bookkeeping.

Momentum assignment rule (component-wise):

- Target extracted momentum: `P = [Px, Py, Pz]`
- Choose integer source momenta `p1` and `p2` such that `P = p1 - p2`
- Keep the split as balanced as possible in magnitude

Recommended balanced integer split for each component `Pi`:

$$
p_{1,i}=\operatorname{sgn}(P_i)\left\lceil \frac{|P_i|}{2}\right\rceil,\qquad
p_{2,i}=-\operatorname{sgn}(P_i)\left\lfloor \frac{|P_i|}{2}\right\rfloor
$$

So for `Pz = 4`, use `p1z = 2`, `p2z = -2` (integer and perfectly balanced).
For odd momentum (for example `Pz = 3`), use `p1z = 2`, `p2z = -1`.

Three phases must be distinguished clearly:

- **Source phase**: phase used in each wall inversion (`src_phase_1` for `S1`, `src_phase_2` for `S2`).
- **Sink phase**: inherited from the second source phase by hermitian conjugation of `S2`; because the contraction uses `S2^dagger`, effective sink momentum is `-p2` (a minus sign relative to `src_phase_2`).
- **Extraction phase**: phase multiplied at contraction time for Fourier projection (usually `exp(-i P·x)` with `P = p1 - p2`).

Important: sink phase is not an independent extra inversion input; it is determined by the source phase of the second propagator plus the conjugation sign flip.

Two wall-source propagators and three phases example:
...
t_src = 0
P = [0, 0, 4]  # target extracted momentum

p1 = [int(math.copysign(math.ceil(abs(x) / 2), x)) for x in P]
p2 = [-int(math.copysign(math.floor(abs(x) / 2), x)) for x in P]
# Here: p1=[0,0,2], p2=[0,0,-2], and P = p1 - p2

phase_builder = phase_v2.MomentumPhase(latt_info)

src_phase_1 = phase_builder.getPhase(p1)  # source phase for S1
src_phase_2 = phase_builder.getPhase(p2)  # source phase for S2

sink_phase_eff = phase_builder.getPhase([-p2[0], -p2[1], -p2[2]])
extract_phase = phase_builder.getPhase([-P[0], -P[1], -P[2]])  # for exp(-i P.x)

with dirac.useGauge(gauge):
  propag_1 = core.invert(dirac, "wall", t_src, src_phase_1.data)
  propag_2 = core.invert(dirac, "wall", t_src, src_phase_2.data)
```

In contraction, use `extract_phase` for Fourier projection. The sink-side sign flip is already encoded by using `S2^dagger` (and equivalently by `sink_phase_eff = -p2` in bookkeeping).

Minimal contraction bookkeeping for one momentum channel:

```python
# after two inversions: propag_1 (p1), propag_2 (p2)
P = [p1[0] - p2[0], p1[1] - p2[1], p1[2] - p2[2]]
extract_phase = phase_builder.getPhase([-P[0], -P[1], -P[2]]).data

prop_2_dag = propag_2.data.conj()
prop_1_shift = propag_1  # for nonlocal case, apply covDev repeatedly on this leg

# contraction form: extract_phase * Tr[S2^dagger (gamma5*Gamma_snk) S1_shift (Gamma_src^dagger*gamma5)]
```
...
For local two-point correlators, no spatial shift is needed. For nonlocal operators
with a spatial displacement (for example, Wilson-line-separated bilinears), perform
the displacement by repeated covariant shifts on the propagator using the **pure gauge**.

Direction convention for `covDev`:
- `mu = 0, 1, 2, 3` means `+x, +y, +z, +t`
- `mu = 4, 5, 6, 7` means `-x, -y, -z, -t`

Recommended pattern:

```python
# Keep a separate pure gauge view for shifts
gauge.ensurePureGauge()
gauge.pure_gauge.loadGauge(gauge)

mu = 2  # +z shift; use 6 for -z
prop_shift = propag_1.copy()

for z in range(z_max + 1):
  # 1) contract at current displacement z
  # C[z] = ... contraction using prop_shift ...

  # 2) shift to z+1 (skip after the last point)
  if z < z_max:
    for spin in range(4):
      for color in range(3):
        fermion = prop_shift.getFermion(spin, color)
        fermion_shift = gauge.pure_gauge.covDev(fermion, mu)
        prop_shift.setFermion(fermion_shift, spin, color)
```

Important: do not mix gauge roles. Keep the Dirac inversion on the gauge field intended
for the solver (often smeared), while `covDev` shifts should use the pure gauge links.

When this nonlocal shift is combined with wall-source momentum 2pt:

- Shift only the `S1` leg (the forward line in `S2^dagger ... S1`).
- Keep `S2` unshifted and use `S2^dagger` in contraction.
- Apply extraction phase with the target `P = p1 - p2` at contraction time.

Runtime reminder (common failure mode):

If your loop alternates between

- `with dirac.useGauge(gauge_solver): ...` for inversion, and
- `gauge_pure.pure_gauge.covDev(...)` for shifts,

the active gauge state used by QUDA can change after leaving the solver context.
If you call `covDev` without reloading pure gauge, you may hit:

`ERROR: Gauge field not allocated`

Recommended safety pattern:

```python
# after reading the pure gauge
gauge_pure.toDevice()
...
for mom_idx in range(n_mom):
  ...
  # reload pure gauge right before covDev shift block
  gauge_pure.ensurePureGauge()
  gauge_pure.pure_gauge.loadGauge(gauge_pure)

  # now do covDev shifts safely
  ...
```

In short: load solver gauge for inversion, then explicitly reload pure gauge before each shift block.
...
- `gamma_src` and `gamma_snk` must represent only the physical bilinear operators at source and sink.
- For the `gtg5` channel, define `gamma_src = gamma_snk = gamma_t @ gamma_5`.
- Do not absorb gamma5-hermiticity factors into `gamma_src` or `gamma_snk`.
- The hermiticity gamma5 factors appear only in the contraction formula.

For a meson two-point function after using gamma5-hermiticity:

$$
C(t) \propto \sum_{\vec{x}} \mathrm{Tr}\left[S^\dagger(\vec{x},t;\vec{x}_0,t_0)\,(\gamma_5\,\Gamma_{\mathrm{snk}})\,S(\vec{x},t;\vec{x}_0,t_0)\,(\Gamma_{\mathrm{src}}^\dagger\,\gamma_5)\right]
$$

where `Gamma_src`/`Gamma_snk` are the physical operators, i.e. exactly what `gamma_src`/`gamma_snk` should store.

For example, for a rho two-point correlator:
```python
import cupy as cp
from pyquda_utils import core, phase_v2, gamma

extract_phase = phase_v2.MomentumPhase(latt_info).getPhase([-kx, -ky, -kz], [x0, y0, z0])
...
# Physical source/sink structures
gamma_src_1 = gamma_1
gamma_snk_1 = gamma_1
gamma_src_2 = gamma_2
gamma_snk_2 = gamma_2
...
# gamma5-hermiticity factors are applied in contraction, not in definitions above
rho_2pt_1 = cp.einsum('wtzyx,wtzyxjiba,jk,wtzyxklba,li->t', extract_phase.data, propag_sh.data.conj(), gamma_5 @ gamma_snk_1, propag_sh.data, gamma_src_1.conj().T @ gamma_5)
rho_2pt_2 = cp.einsum('wtzyx,wtzyxjiba,jk,wtzyxklba,li->t', extract_phase.data, propag_sh.data.conj(), gamma_5 @ gamma_snk_2, propag_sh.data, gamma_src_2.conj().T @ gamma_5)
rho_2pt_3 = cp.einsum('wtzyx,wtzyxjiba,jk,wtzyxklba,li->t', extract_phase.data, propag_sh.data.conj(), gamma_5 @ gamma_snk_3, propag_sh.data, gamma_src_3.conj().T @ gamma_5)

# Explicit gtg5 example
gamma_t = gamma.gamma(8)
gamma_src_gtg5 = gamma_t @ gamma_5
gamma_snk_gtg5 = gamma_t @ gamma_5
# In einsum use: (gamma_5 @ gamma_snk_gtg5) and (gamma_src_gtg5.conj().T @ gamma_5)
...
```
Please refer to "lqcd-physics" skills for more details on how to perform contractions for various hadronic correlators. The convention for the spin-color indices in the propagator is (parity, t, z, y, x, spin_snk, spin_src, color_snk, color_src). The `einsum` function is used to perform the necessary contractions to compute the two-point correlator for the rho meson. The `extract_phase.data` is the extraction phase used at contraction to project $e^{-i\vec{p}\cdot\vec{x}}$. In wall-source momentum 2pt with two propagators, source phases are set at inversion, sink phase comes from hermitian conjugation of the second line (`S2^dagger`, hence one minus sign relative to the second source phase), and extraction phase is applied only at contraction. Finally, we gather the lattice data from all MPI ranks to obtain the full correlator in the root rank as a function of time. The second argument of `gatherLattice` specifies the dimensions to gather in `tzyx` order, where `0` indicates that we want to gather the data in the $t$ dimension, and `-1` means performing reduction in all three spatial dimensions.
...
- **Grid is not initialized**: You likely called `io.read*Gauge(...)` before `core.init(...)`. Move `core.init` and `LatticeInfo` creation to the top of the script, before any gauge read.
